...
// it is in its correct location in the heap.
{
	...
	// ************ <your code here> *****************************
	   int temp, parent, index = max_heap_peek(source);
	   int val = source->values[index];

	   while(index)
	   {
		   parent = (index - 1) / 2;
		   if (parent < 0 || source->values[parent] >= val) break;
		   else {
			   temp = source->values[index];
			   source->values[index] = source->values[parent];
			   source->values[parent] = temp;
			   index = parent;
		   }
