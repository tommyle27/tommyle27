...
//==============================================
{
	...
	avl_node *p = temp->left;
	t->left = node;
	node->right = p;

	// < your code here >
	node->height = MAX_HEIGHT(avl_node_height(node->left), avl_node_height(node->right)) +1;
	t->height = MAX_HEIGHT(avl_node_height(t->left), avl_node_height(t->right)) + 1;
...
//================================================
{
	...
	// < your code here >
	if(node == NULL || node->height == 1) {
		if(avl_balanced(tree)) { is_valid = 1;}
		else if (node->data < min_node->data == node->data > max_node->data) {
			is_valid = 0;
		} else {
			is_valid = avl_vlaid_aux(tree, node->left, node->left->left, node->left->right && avl_valid_aux(tree, node->right, node->right->left, node->right->right));
		}
	}
