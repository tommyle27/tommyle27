...
void* threadRun(void *t) //implement this function in a suitable way
{
	...
	char dup[4];
	strncpy(dup,((Thread*)t)->tid, 4); //get the Y value
	int idTwo = atoi(&dup[2]);


	sem_wait(&running);
	if (idTwo % 2 == 0) { //check if even or odd for wait
		sem_wait(&even); //decrement semaphore value
	}
	...
	if (idTwo % 2 == 0) { //check if even or odd for post
		sem_post(&even); //increment value
	}
